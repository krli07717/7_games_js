...
const checkBoard = () => {
    const winArray = [[0, 1, 2, 3], [41, 40, 39, 38], [7, 8, 9, 10], [34, 33, 32, 31], [14, 15, 16, 17], [27, 26, 25, 24], [21, 22, 23, 24],
    ...
    for (let [s1,s2,s3,s4] of winArray) {
        if (squares[s1].classList.contains("player-one") &&
            squares[s2].classList.contains("player-one") &&
            squares[s3].classList.contains("player-one") &&
            squares[s4].classList.contains("player-one") 
        ) {
            result.innerHTML="Player-one wins"
        } else if (
            squares[s1].classList.contains("player-two") &&
            squares[s2].classList.contains("player-two") &&
            squares[s3].classList.contains("player-two") &&
            squares[s4].classList.contains("player-two") 
        ) {
            result.innerHTML="Player-one wins"
        }
    }
    ...
    for (let i = 0; i < squares.length; i++) {
        squares[i].addEventListener("click", () => {
            if (!squares[i].classList.contains("taken")) {
                if (squares[i].classList.contains("buttom") ||
                (squares[i+7] && squares[i+7].classList.contains("taken"))) {
                    if (Player === 1) {
        ...
        squares[i].addEventListener("click",checkBoard)
    }
}
